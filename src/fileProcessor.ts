...
export class FileProcessor {
    ...
    for (const rule of applicableRules) {
      ...
      this.fileChangeDebounce.set(
        ...
        setTimeout(() => {
          ...
          console.log("eRouter486: processFile called");
  ...
  private async processFile(file: TFile, rule: MonitoringRule): Promise<void> {
    ...
    if (this.processingFiles.has(file.path)) {
      console.log(
        `eRouter486: File ${file.path} is already being processed. Skipping.`,
      );
      return;
    }
    ...
    console.log(
      `eRouter486: Processing file ${file.path} with rule ${JSON.stringify(rule)}`,
    );
    ...
    try {
      if (await this.app.vault.adapter.exists(file.path)) {
        const content = await this.app.vault.read(file);
        console.debug(
          `eRouter486: File content read, length: ${content.length}`,
        );
        ...
        console.debug(
          `eRouter486: Received processed content from LLM, length: ${processedContent.length}`,
        );

        if (content === processedContent) {
          console.debug(
            `eRouter486: LLM did not make any changes to the content`,
          );
          return;
        }

        console.debug(`eRouter486: Now attempting to save processed content`);
        const outputFileName = await this.saveProcessedContent(
          file,
          processedContent,
          rule,
        );
        console.debug(
          `eRouter486: saveProcessedContent returned: ${outputFileName}`,
        );

        if (outputFileName) {
          console.debug(
            `eRouter486: Content saved successfully to ${outputFileName}`,
          );

          if (
            rule.templateFile &&
            (await this.app.vault.adapter.exists(rule.templateFile))
          ) {
            console.debug(
              `eRouter486: Using template file ${rule.templateFile}`,
            );
            ...
            console.debug(
              `eRouter486: Template content read, length: ${templateContent.length}`,
            );
            ...
            if (templater) {
              console.debug(
                `eRouter486: Templater plugin found, applying template`,
              );
              const outputFile = this.app.vault.getAbstractFileByPath(
                outputFileName,
              ) as TFile;
              try {
                await templater.templater.overwrite_file_commands(
                  outputFile,
                  templateContent,
                );
                console.debug(
                  `eRouter486: Template successfully applied to output file using Templater`,
                );
              } catch (error) {
                console.error(
                  `eRouter486: Error applying template with Templater:`,
                  error,
                );
                console.debug(
                  `eRouter486: Falling back to manual template application`,
                );
                await this.manualTemplateApplication(
                  outputFile,
                  templateContent,
                  processedContent,
                );
              ...
              console.warn(
                "eRouter486: Templater plugin not found or not accessible. Applying template manually.",
              );
              const outputFile = this.app.vault.getAbstractFileByPath(
                outputFileName,
              ) as TFile;
              if (outputFile instanceof TFile) {
                ...
                console.error(
                  `eRouter486: Output file not found: ${outputFileName}`,
                );
              }
            }
            ...
            console.debug(
              `eRouter486: No template file used or file doesn't exist`,
            );
          }
          ...
          if (rule.deleteSourceFile) {
            console.debug(
              `eRouter486: Attempting to delete source file ${file.path}`,
            );
            try {
              if (await this.app.vault.adapter.exists(file.path)) {
                await this.app.vault.delete(file);
                console.debug(
                  `eRouter486: Source file ${file.path} deleted successfully`,
                );
                await this.logOperation("delete", file.path, rule);
              } else {
                console.warn(
                  `eRouter486: Source file ${file.path} no longer exists. Skipping deletion.`,
                );
              }
            } catch (error) {
              console.error(
                `eRouter486: Failed to delete source file ${file.path}:`,
                error,
              );
            }
          }
        ...
        console.warn(
          `eRouter486: File ${file.path} does not exist. Skipping processing.`,
        );
      }
    } catch (error) {
  ...
  private monitorFolder(folder: string, rule: MonitoringRule): void {
    ...
    const watcher = setInterval(
      () => this.checkFolder(folder, rule),
      rule.delay * 1000,
    );
    this.fileWatchers.set(`${folder}-${rule.fileNameTemplate}`, watcher);
    console.debug(
      `ERouter486Plugin: Started monitoring folder ${folder} with template ${rule.fileNameTemplate}`,
    );
  }
  ...
  private async manualTemplateApplication(
    outputFile: TFile,
    templateContent: string,
    processedContent: string,
  ): Promise<void> {
    console.debug(
      `eRouter486: Starting manual template application for file ${outputFile.path}`,
    );
    console.debug(
      `eRouter486: Template content length: ${templateContent.length}`,
    );
    console.debug(
      `eRouter486: Processed content length: ${processedContent.length}`,
    );
    try {
      const combinedContent = templateContent + "\n" + processedContent;
      console.debug(
        `eRouter486: Combined content length: ${combinedContent.length}`,
      );
      await this.app.vault.modify(outputFile, combinedContent);
      console.debug(
        `eRouter486: Manual template application completed for file ${outputFile.path}`,
      );
    } catch (error) {
      console.error(
        `eRouter486: Error in manual template application for file ${outputFile.path}:`,
        error,
      );
    }
  }
  ...
  async queueLLMRequest(content: string, prompt: string): Promise<string> {
    console.debug(
      `eRouter486: Queueing LLM request. Content length: ${content.length}, Prompt: ${prompt}`,
    );
  ...
  private async processQueue() {
    ...
    while (this.requestQueue.length > 0) {
      ...
      if (timeSinceLastRequest < this.REQUEST_INTERVAL) {
        ...
        console.debug(
          `eRouter486: Rate limiting - Waiting ${waitTime}ms before processing next request`,
        );
  ...
  async processWithLLM(content: string, prompt: string): Promise<string> {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        console.debug(
          `eRouter486: Making LLM request (attempt ${attempt}/${maxRetries})`,
        );
        ...
        const response =
          chatCompletion.choices[0]?.message?.content || "No response from LLM";
        console.debug(`eRouter486: LLM response length: ${response.length}`);
        return response;
      } catch (error) {
        console.error(
          `eRouter486: Error processing with LLM (attempt ${attempt}/${maxRetries}):`,
          error,
        );
        ...
        if ((error as Error).message.includes("429")) {
          console.debug(
            `eRouter486: Rate limit reached. Waiting ${delayBetweenRetries}ms before retrying...`,
          );
    ...
    while ((match = linkRegex.exec(text)) !== null) {
      ...
      if (file instanceof TFile) {
        try {
          ...
          new Notice(
            `Error reading file ${fileName}. The link will be left as is.`,
          );
        }
      } else {
  ...
  async saveProcessedContent(
    ...
    const outputFileName = this.getOutputFileName(
      file.name,
      rule.outputFileNameTemplate,
    );
    console.debug(
      `eRouter486: Attempting to save content to ${outputFileName}`,
    );

    try {
      const outputFile = this.app.vault.getAbstractFileByPath(outputFileName);
      console.debug(
        `eRouter486: Output file exists: ${outputFile instanceof TFile}`,
      );

      if (outputFile instanceof TFile) {
        console.debug(
          `eRouter486: Output file handling: ${rule.outputFileHandling}`,
        );
        ...
        console.debug(
          `eRouter486: Output file doesn't exist, creating new file ${outputFileName}`,
        );
      ...
      console.debug(
        `eRouter486: Successfully saved processed content to ${outputFileName}`,
      );
  ...
  async logOperation(
    ...
    if (outputFileName) {
      ...
      console.debug(
        `eRouter486: No output file created or modified for ${filePath}`,
      );
    }
  }
