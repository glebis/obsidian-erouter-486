...
export default class ERouter486Plugin extends Plugin {
  ...
  async onload() {
    console.log("Loading eRouter486");
    ...
    this.logActiveRules();
    console.log("eRouter486: Plugin loaded and monitoring started");
  }

  onunload() {
    this.fileProcessor.stopFileMonitoring();
    console.debug("eRouter486: Plugin unloaded and monitoring stopped");
  }
  ...
  logActiveRules() {
    const activeRules = this.settings.monitoringRules.filter(
      (rule) => rule.enabled,
    );
  ...
  async testLLMConnection(): Promise<{ success: boolean; message: string }> {
    ...
    try {
      ...
      if (chatCompletion.choices && chatCompletion.choices.length > 0) {
        ...
        return {
          success: false,
          message: "Connection to GROQ failed: No response received.",
        };
      }
    } catch (error) {
      ...
      return {
        success: false,
        message: `Connection to GROQ failed: ${(error as Error).message}`,
      };
    }
  }
